 ...
 * See how the 7B model performs with large context
 */

const ROUTER_URL = 'http://192.168.0.21:3000';
const API_KEY = 'test-key';

async function test32kContext() {
  console.log('\n🧪 Testing with ~32k token context\n');
  
  const messages = [
    { role: 'system', content: 'You are a helpful AI assistant.' }
  ];
  
  // Add ~160 message pairs (~32000 tokens total)
  for (let i = 0; i < 160; i++) {
    messages.push(
      { 
        role: 'user', 
  ...
  console.log(`📊 Estimated tokens: ~32000`);
  ...
  try {
    ...
    if (elapsed < 3000) {
      ...
      console.log('   ❌ TOO SLOW - Response took 30-60 seconds');
    } else {
      console.log('   ❌ UNACCEPTABLE - Response took over 1 minute');
    }
    
    // Calculate tokens per second
    const estimatedTokens = 32000;
...
// Run test
console.log('═══════════════════════════════════════');
console.log('  32K Context Speed Test');
console.log('═══════════════════════════════════════');

test32kContext().then(success => {
  console.log('═══════════════════════════════════════');
  if (success) {
